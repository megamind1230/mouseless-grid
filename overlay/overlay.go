...
func New() (*Overlay, error) {
	...
	// baka: value list ordered by mask bit position (LSB→MSB): BackPixmap(0), BorderPixel(3), EventMask(11), Colormap(13)
	// NO override-redirect — let i3 manage stacking
	...
	// set properties before map
	setWindowType(conn, wid)
	setAlwaysOnTopProperty(conn, wid)
	setSticky(conn, wid) // all desktops
...
func (o *Overlay) Render(img *image.NRGBA) error {
	...
	// baka: max X11 request payload ~256KB; split into horizontal strips
	// 256KB / (1366 * 4 bytes/pixel) ≈ 47 rows per strip
	maxBytes := 256 * 1024
	rowBytes := w * 4
...
// setSticky sets _NET_WM_DESKTOP to 0xFFFFFFFF (all desktops)
